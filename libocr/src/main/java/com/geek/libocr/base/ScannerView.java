...
//                    barcode.setCrop(rect.left, rect.top, rect.width(), rect.height());
            ...
            if (enableIdCard && result == null) {
                try {
                    if (!isIdCardInit) {
                        isIdCardInit = IdCardUtils.initDict(getContext());
                    }
                    if (!isIdCardInit) {
                        throw new Exception("failure");
                    }
                    ...
                    if (len > 0) {
                        result = Utils.decodeIdCard(obtain, len);
                    }
                } catch (Exception e) {
                    Log.e(TAG, e.getMessage());
                }
...
//                    if (!TextUtils.isEmpty(s)) {
//                        result = new com.shouzhong.scanner.Result();
...
//                    if (!TextUtils.isEmpty(s)) {
//                        result = new com.shouzhong.scanner.Result();
//                        result.type = com.shouzhong.scanner.Result.TYPE_DRIVING_LICENSE;
//                        result.data = s;
//                    }
//                } catch (Exception e) {
//                    Log.e(TAG, e.getMessage());
...
//            }
    ...
    public void setAutoFocusArea() {
        //设置对焦区域
        if (!shouldAdjustFocusArea || cameraWrapper == null) {
            return;
        }
        try {
...
//        }
    ...
    private Rect getScaledRect(int previewWidth, int previewHeight) {
        if (scaledRect == null) {
            ...
            if (scaledRect.right > previewWidth) {
                scaledRect.right = previewWidth;
            }
            if (scaledRect.bottom > previewHeight) {
                scaledRect.bottom = previewHeight;
            }
        }
        return scaledRect;
    }
    ...
    void setOptimalPreviewSize() {
        if (previewSize != null || cameraWrapper == null) {
            return;
        }
        int w, h;
