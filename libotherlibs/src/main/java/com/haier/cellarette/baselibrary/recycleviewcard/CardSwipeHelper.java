...
public class CardSwipeHelper implements View.OnTouchListener {

    private final CardSwipeStack mSwipeStack;
    ...
    private float mOpacityEnd = DEFAULT_SWIPE_OPACITY;
    private int mAnimationDuration = DEFAULT_ANIMATION_DURATION;

    public CardSwipeHelper(CardSwipeStack swipeStack) {
    ...
    private void checkViewPosition() {
        ...
        if (viewCenterHorizontal < parentFirstThird &&
                mSwipeStack.getAllowedSwipeDirections() != SWIPE_DIRECTION_ONLY_RIGHT) {
            swipeViewToLeft(mAnimationDuration / 2);
        } else if (viewCenterHorizontal > parentLastThird &&
                mSwipeStack.getAllowedSwipeDirections() != SWIPE_DIRECTION_ONLY_LEFT) {
    ...
    private void swipeViewToLeft(int duration) {
        ...
        mObservedView.animate()
                ...
                .setListener(new CardAnimationUtils.AnimationEndListener() {
                    @Override
