...
public class StackLayoutManager extends RecyclerView.LayoutManager {
    ...
    private StackAlign direction = LEFT;
    private RecyclerView mRV;
    private Method sSetScrollState;
    private int mPendingScrollPosition = NO_POSITION;

    StackLayoutManager(StackConfig config, Context context) {
        this();
    ...
    private int fill(RecyclerView.Recycler recycler, int dy, boolean apply) {
        ...
        if (direction == TOP) {
            ...
            return dy;//bottom alignment is not necessary,we don't support that
        }
    }
...
//                        -(position - currPos)*(mItemWidth) * (1 - closestBaseItemScale));
    ...
    private class H extends Handler {
        ...
        public H(StackLayoutManager mReference) {
            this.mReference = new WeakReference<StackLayoutManager>(mReference);
        }
        ...
        public void handleMessage(Message msg) {
            ...
            if (msg.what == MSG_RUN && isScrolling) {
                if (!isTouch) {
                    int pos = mReference.get().getPos() + 1;
                    Log.e("ssssssssssssssss", pos + "");
                    mReference.get().scrollToPosition(pos);
//                    mHandler.removeMessages(MSG_RUN);
                    Message msg1 = mHandler.obtainMessage(MSG_RUN, isScrolling ? 1 : 0, isTouch ? 1 : 0);
                    sendMessageDelayed(msg1, mScrollTime);
                } else {
